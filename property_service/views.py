...
class SearchPropertyView(APIView):
    ...
    def get(self, request, format=None):
        try:
           ...
           return Response(
               {"success":"Data processed"}, status=status.HTTP_200_OK
           )
        except:
            return Response(
...
#  agent = request.query_params.get("agent_name")
#             search = request.query_params.get("search")
...
#             if (
#                 not Property.objects.annotate(search=vector)
#                 .filter(search=query, agent=agent, is_publised=True)
#                 .exists()
#             ):
#                 return Response(
#                     {"error": "No property found with such details"},
#                     status=status.HTTP_404_NOT_FOUND,
#                 )

#             properties = Property.objects.annotate(search=vector()).filter(
#                 search=query, agent=agent, is_published=True
#             )
#             properties = PropertySerializer(properties, many=True)

#             return Response({"Properties": properties.data}, status=status.HTTP_200_OK)